...
router.get("/private", authToken, async (req, res) => {
  try {
    ...
    console.log("🚀 ~ file: session.routes.js:18 ~ router.get ~ decodedToken:", decodedToken);
...
router.post("/register", async (req, res) => {
  try {
    ...
    console.log("🚀 ~ file: session.routes.js:63 ~ router.post ~ token:", token);

    req.session.user = { email, first_name, last_name, age, token };
    return res.render(`login`);
  } catch (error) {
    console.log("🚀 ~ file: session.routes.js:36 ~ router.post ~ error:", error);
  }
});
...
router.post("/update", async (req, res) => {
  try {
    ...
    console.log("🚀 ~ file: session.routes.js:97 ~ router.post ~ error:", error);
  }
});
