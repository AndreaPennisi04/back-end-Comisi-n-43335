...
router.post("/login", async (req, res) => {
  try {
    ...
    if (!email || !password) {
      return res.status(400).json({ status: "error", error: "Errors in Body for LOGIN" });
    }
    const findUser = await userModel.findOne({ email });

    if (!findUser) {
      return res.status(404).json({ status: "error", message: `this user does not exists` });
    }
    ...
    console.log("🚀 ~ file: session.routes.js:34 ~ router.post ~ error:", error);
  }
});
...
router.post("/register", async (req, res) => {
  try {
    ...
    console.log("🚀 ~ file: session.routes.js:73 ~ router.post ~ error:", error);
  }
});
