...
router.get("/:petName([a-zA-Z]+)", (req, res) => {
  ...
  const findPet = pets.find((pet) => pet.name.toLowerCase() === petName.toLowerCase());

  if (!findPet) {
    return res.status(404).json({ message: `this pet ${petName} does not exist` });
  }
...
router.put("/:petName([a-zA-Z]+)", (req, res) => {
  try {
    const { petName } = req;
    const infoPet = pets.find((pet) => pet.name.toLowerCase() === petName.toLowerCase());
...
//no entiendo a que van estas lineas
