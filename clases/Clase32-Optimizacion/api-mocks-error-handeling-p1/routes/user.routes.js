...
router.get("/", async (req, res) => {
  try {
    ...
    return httpResp.Error(res, `Something wrong happens generating users`, error?.message);
  }
});
...
router.get("/:uid", async (req, res) => {
  try {
    ...
    if (!uid || isNaN(uid) || uid < 0) {
      return httpResp.BadRequest(res, `${EnumErrors.INVALID_PARAMS} - Invalid Params for userId `, req.params);
    }
