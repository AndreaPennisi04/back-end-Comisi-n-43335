...
router.get("/:noteId", [isValidMongoId("noteId"), handlePolicies(["USER"])], async (req, res) => {
...
router.put("/:noteId", [isValidMongoId("noteId"), handlePolicies(["USER"])], async (req, res) => {
  // TODO: VALIDAR QUIEN CREO LA NOTA COMO EJERCICIO Y SI EL MISMO QUE LA VA MODIFICAR
});

router.delete("/:noteId", [isValidMongoId("noteId"), handlePolicies(["ADMIN"])], async (req, res) => {
  const { noteId } = req.params;
