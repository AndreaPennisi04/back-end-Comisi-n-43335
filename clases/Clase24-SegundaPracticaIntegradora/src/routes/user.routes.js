...
router.get("/:uId", [isValidMongoId("uId"), handlePolicies(["ADMIN", "USER"])], async (req, res) => {
  const { uId } = req.params;
  const userData = await userModel.findById({ _id: uId }).populate("notes.note");
...
router.delete("/:uId", [isValidMongoId, handlePolicies(["ADMIN"])], async (req, res) => {
  const { uId } = req.params;
...
router.post(
  ...
  async (req, res) => {
    ...
    console.log("🚀 ~ file: user.routes.js:48 ~ updateUserNote:", updateUserNote);
